...
def patch_library():
    if len(libios[1]) != 0:
        buffer = open('Flutter', 'rb').read().replace(
            b'192.168.133.104', IPBurp.encode('ascii'))
        open('Flutter', 'wb').write(buffer)
    if len(libAppArm64[1]) != 0:
        buffer = open('libflutter_arm64.so', 'rb').read().replace(
            b'192.168.133.104', IPBurp.encode('ascii'))
        open('libflutter_arm64.so', 'wb').write(buffer)
    if len(libAppArm[1]) != 0:
        buffer = open('libflutter_arm.so', 'rb').read().replace(
            b'192.168.133.104', IPBurp.encode('ascii'))
        open('libflutter_arm.so', 'wb').write(buffer)
    if len(libAppX64[1]) != 0:
        buffer = open('libflutter_x64.so', 'rb').read().replace(
            b'192.168.133.104', IPBurp.encode('ascii'))
...
def network_lib():
    ...
    if len(libios[1]) != 0:
        try:
            urlretrieve("https://github.com/ptswarm/reFlutter/releases/download/ios-" +
                        libios[1]+"/Flutter", "Flutter")
        except:
            libios = '', ''
            not_except("Flutter")
    if len(libAppArm64[1]) != 0:
        try:
            urlretrieve("https://github.com/ptswarm/reFlutter/releases/download/android-" +
                        libAppArm64[1]+"/libflutter_arm64.so", "libflutter_arm64.so")
        except:
            libAppArm64 = '', ''
    ...
    if len(libAppArm[1]) != 0:
        try:
            urlretrieve("https://github.com/ptswarm/reFlutter/releases/download/android-" +
                        libAppArm[1]+"/libflutter_arm.so", "libflutter_arm.so")
        except:
            libAppArm = '', ''
    ...
    if len(libAppX64[1]) != 0:
        try:
            urlretrieve("https://github.com/ptswarm/reFlutter/releases/download/android-" +
                        libAppX64[1]+"/libflutter_x64.so", "libflutter_x64.so")
        except:
            libAppX64 = '', ''
    ...
    if len(libAppX86[1]) != 0:
        try:
            urlretrieve("https://github.com/ptswarm/reFlutter/releases/download/android-" +
                        libAppX86[1]+"/libflutter_x86.so", "libflutter_x86.so")
        except:
            libAppX86 = '', ''
        ...
        for file in files:
            if type(file) is str:
                if file.endswith('.so') and ZIPSTORED or file.endswith('resources.arsc'):
                    ziph.write(os.path.join(root, file), os.path.relpath(os.path.join(root.replace(
                        'release/', ''), file), os.path.join(path, '..')), zipfile.ZIP_STORED)
                else:
                    ziph.write(os.path.join(root, file), os.path.relpath(os.path.join(root.replace(
                        'release/', ''), file), os.path.join(path, '..')), zipfile.ZIP_DEFLATED)
            else:
                ziph.write(os.path.join(root, file), os.path.relpath(os.path.join(root.replace(
                    'release/', ''), file), os.path.join(path, '..')), zipfile.ZIP_DEFLATED)


def replace_libflutter():
    if len(sys.argv) < 3:
        check_hash()
        input_IPBurp()
        network_lib()
    if os.path.exists("libflutter_arm64.so") or os.path.exists("libflutter_arm.so") or os.path.exists("libflutter_x64.so") or os.path.exists("libflutter_x86.so") or os.path.exists("Flutter"):
        try:
            shutil.move("Flutter", join("release", libios[0].replace(
                "App.framework/App", "Flutter.framework/Flutter").replace("FlutterApp.framework/FlutterApp", "Flutter.framework/Flutter")))
        except:
            pass
        try:
            shutil.move("libflutter_arm64.so", join(
                "release", libAppArm64[0].replace("libapp.so", "libflutter.so")))
        except:
            pass
        try:
            shutil.move("libflutter_arm.so", join(
                "release", libAppArm[0].replace("libapp.so", "libflutter.so")))
        except:
            pass
        try:
            shutil.move("libflutter_x64.so", join(
                "release", libAppX64[0].replace("libapp.so", "libflutter.so")))
        except:
            pass
        try:
...
def patch_source(hashS, ver):
    replace_file_text('src/third_party/dart/runtime/vm/dart.cc',
                      'FLAG_print_class_table)', 'true)')
    if ver > 27:
    ...
    replace_file_text('src/third_party/dart/tools/make_version.py',
                      'snapshot_hash = MakeSnapshotHashString()', 'snapshot_hash = \''+hashS+'\'')
    replace_file_text('src/third_party/dart/runtime/bin/socket.cc', 'DartUtils::GetInt64ValueCheckRange(port_arg, 0, 65535);',
    ...
    replace_file_text('src/third_party/boringssl/src/ssl/ssl_x509.cc',
    ...
    if ver == 26 or ver == 27:
        replace_file_text('tools/generate_package_config/pubspec.yaml',
                          'package_config: any', 'package_config: 1.9.3')
    if ver == 24:
        replace_file_text(
            'DEPS', 'flutter_internal/android/sdk/licenses', 'flutter/android/sdk/licenses')
    if ver == 14 or ver == 13:
        replace_file_text(
    ...
    if ver == 11 or ver == 10 or ver == 9 or ver == 8:
        replace_file_text(
            "DEPS", "   'src/third_party/dart/tools/sdks': {\n     'packages': [\n       {\n         'package': 'dart/dart-sdk/${{platform}}',\n         'version': 'version:2.4.0'\n       }\n     ],\n     'dep_type': 'cipd',\n   },\n", "")
        replace_file_text(
